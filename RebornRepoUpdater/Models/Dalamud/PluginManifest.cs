...
public class PluginManifest
{
	...
	public void SetDownloadCount(IEnumerable<Release> releases)
	{
		...
		if (newCount > DownloadCount)
			...
			Console.WriteLine($"WARNING: Computed download count ({newCount}) is not greater than existing count ({DownloadCount}). Preserving existing count to avoid data loss.");
	}
}